...
interface InternsPageProps {
  ...
  onViewAllInterns?: () => void;
}

export default function InternsPage({ currentUser, onViewAssignedCompanies, onViewAllInterns }: InternsPageProps) {
  ...
  useEffect(() => {
    filterInterns();
  }, [searchQuery, selectedSchoolYear, interns, showAllInterns]);
  ...
  }, [showAssignedCompanyModal, slideInAnim]);
  ...
  const filterInterns = () => {
    ...
    // Filter by school year only if not showing all interns
    if (!showAllInterns) {
  ...
  // Generate school year options (starting from 2023-2024)
  ...
  const handleCreateClass = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Optionally refresh classes list if needed
        // You can add a function to fetch and display classes here
      } else {
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={[
          ...
          <View style={[
            ...
            >
              ...
              />
              <Text style={[styles.addButtonText, { fontSize: responsiveStyles.buttonTextSize }]}>
                {dimensions.width < 768 ? 'Excel' : 'List'}
              </Text>
            </TouchableOpacity>
            ...
            {onViewAllInterns ? (
              <TouchableOpacity 
                style={[styles.addButton, styles.allInternsButton]}
                onPress={() => {
                  if (onViewAllInterns) {
                    onViewAllInterns();
                  }
              ...
              >
                ...
                />
                <Text style={[styles.addButtonText, { fontSize: responsiveStyles.buttonTextSize }]}>
                  {dimensions.width < 768 ? 'All List' : 'All Interns List'}
                </Text>
              </TouchableOpacity>
            ) : (
              <TouchableOpacity 
                style={[
                  styles.addButton, 
                  showAllInterns ? styles.allInternsButtonActive : styles.allInternsButton
                ]}
                onPress={() => setShowAllInterns(!showAllInterns)}
              >
                <MaterialIcons 
                  name={showAllInterns ? "list" : "filter-list"} 
                  size={responsiveStyles.buttonIconSize} 
                  color="#fff" 
                />
                <Text style={[styles.addButtonText, { fontSize: responsiveStyles.buttonTextSize }]}>
                  {dimensions.width < 768 
                    ? (showAllInterns ? 'Filtered' : 'All List') 
                    : (showAllInterns ? 'Show Filtered' : 'All Interns List')
                  }
                </Text>
              </TouchableOpacity>
            )}
      ...
      {/* Attendance History Modal - Full Screen */}
      ...
      {/* Assigned Company Slide-In Panel */}
      {showAssignedCompanyModal && (
        <View style={styles.slideInOverlay}>
          <TouchableOpacity 
            style={styles.slideInBackdrop} 
            activeOpacity={1} 
          ...
          >
            <View style={styles.slideInHeader}>
              <TouchableOpacity style={styles.slideInBackButton} onPress={() => setShowAssignedCompanyModal(false)}>
                <MaterialIcons name="arrow-back" size={24} color="#1E3A5F" />
              </TouchableOpacity>
              <Text style={styles.slideInTitle}>Assigned Companies</Text>
              <Text style={styles.slideInSubtitle}>{assignedCompanies.length} {assignedCompanies.length === 1 ? 'Company' : 'Companies'}</Text>
            </View>

            <View style={styles.slideInContentContainer}>
              {loadingCompany ? (
                <View style={styles.slideInLoadingContainer}>
                  <ActivityIndicator size="large" color="#F56E0F" />
                  <Text style={styles.slideInLoadingText}>Loading company information...</Text>
                </View>
              ) : assignedCompanies.length > 0 ? (
                <ScrollView 
                  style={styles.slideInTableContainer}
                  contentContainerStyle={styles.slideInTableContainerContent}
                  showsVerticalScrollIndicator={true}
                  nestedScrollEnabled={true}
                >
                  <ScrollView
                    ...
                    nestedScrollEnabled={true}
                  >
                    <View style={styles.slideInTable}>
                      {/* Table Header */}
                      <View style={styles.slideInTableHeader}>
                        <View style={[styles.slideInHeaderCell, styles.slideInDateCell]}>
                          <Text style={styles.slideInHeaderText}>#</Text>
                        </View>
                        <View style={[styles.slideInHeaderCell, styles.slideInCompanyCell]}>
                          <Text style={styles.slideInHeaderText}>Company Name</Text>
                        </View>
                        <View style={[styles.slideInHeaderCell, styles.slideInIndustryCell]}>
                          <Text style={styles.slideInHeaderText}>Industry</Text>
                        </View>
                      </View>

                      {/* Table Rows */}
                      {assignedCompanies.map((company, index) => (
                        <View key={company.id || index} style={[
                          styles.slideInTableRow,
                          index % 2 === 0 ? styles.slideInTableRowEven : styles.slideInTableRowOdd,
                        ]}>
                          <View style={[styles.slideInTableCell, styles.slideInDateCell]}>
                            <Text style={styles.slideInCellText}>{index + 1}</Text>
                          </View>
                          <View style={[styles.slideInTableCell, styles.slideInCompanyCell]}>
                            <View style={styles.slideInCompanyNameCell}>
                              {company.company_profile_picture ? (
                                <Image 
                                  source={{ uri: company.company_profile_picture }} 
                                  style={styles.slideInTableCompanyImage}
                                />
                              ) : (
                                <View style={styles.slideInTableCompanyImagePlaceholder}>
                                  <MaterialIcons name="business" size={16} color="#F56E0F" />
                                </View>
                              )}
                              <Text style={styles.slideInCellText} numberOfLines={2}>
                                {company.company_name}
                              </Text>
                            </View>
                          </View>
                          <View style={[styles.slideInTableCell, styles.slideInIndustryCell]}>
                            <Text style={styles.slideInCellText} numberOfLines={2}>
                              {company.industry}
                            </Text>
                          </View>
                ...
                <View style={styles.slideInEmptyContainer}>
                  <MaterialIcons name="business" size={64} color="#ccc" />
                  <Text style={styles.slideInEmptyText}>No Company Assigned</Text>
                  <Text style={styles.slideInEmptySubtext}>
                    This student has not been assigned to any company yet.
                  </Text>
                </View>
              )}
...
const styles = StyleSheet.create({
  ...
  allInternsButton: {
    backgroundColor: '#4285f4', // Blue for all interns list
    elevation: 3,
    shadowColor: '#4285f4',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.3,
  ...
  allInternsButtonActive: {
    backgroundColor: '#9c27b0', // Purple when active (showing filtered)
    elevation: 3,
    shadowColor: '#9c27b0',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.3,
  ...
  // Slide-In Panel Styles for Assigned Companies
  slideInOverlay: {
    position: 'absolute',
    top: 0,
    left: 0,
  ...
  slideInPanel: {
    ...
    top: 0,
    right: 0,
    width: isMobile ? screenWidth : Math.min(800, screenWidth * 0.8),
    height: screenHeight,
    backgroundColor: '#fff',
    elevation: 10,
    shadowColor: '#000',
  ...
  slideInTableContainer: {
    flex: 1,
  },
  slideInTableContainerContent: {
    paddingBottom: 20,
  },
  slideInTable: {
  ...
  slideInCompanyNameCell: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
    width: '100%',
  },
  slideInTableCompanyImage: {
    width: 32,
    height: 32,
    borderRadius: 16,
  },
  slideInTableCompanyImagePlaceholder: {
    width: 32,
    height: 32,
    borderRadius: 16,
    backgroundColor: 'rgba(245, 110, 15, 0.2)',
