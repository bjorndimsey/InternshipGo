...
interface Company {
  ...
  partnerships?: Partnership[]; // NEW: Array of all partnerships
}
...
export default function CompaniesPage({ currentUser }: CompaniesPageProps) {
  ...
  const fetchAllCompanies = async () => {
    try {
      ...
      console.log('🔍 Fetching companies with partnerships...');
      
      // Use getAllCompanies() to get all companies without coordinator filter
      const response = await apiService.getAllCompanies();
      
      console.log('📡 All companies API response:', response);
      
      if (response.success && response.companies) {
        console.log('✅ All companies fetched successfully:', response.companies.length);
        
        // Transform and filter companies - only those with partnership status
        const transformedCompanies: Company[] = [];
        ...
        for (const company of response.companies) {
          ...
          // Only include companies that have some partnership status (not completely pending)
          // This means at least one side has approved OR has MOA uploaded by a coordinator OR has partnerships
          const hasPartnerships = company.partnerships && Array.isArray(company.partnerships) && company.partnerships.length > 0;
          const hasActivePartnership = hasPartnerships || coordinatorApproved || companyApproved || company.moa_uploaded_by;
          
          if (hasActivePartnership) {
            // Get all coordinator names from partnerships array (NEW WAY)
            let coordinatorNames: string[] = [];
            let coordinatorIds: string[] = [];
            
            if (company.partnerships && company.partnerships.length > 0) {
              // Use partnerships from junction table
              coordinatorNames = company.partnerships
                .map((p: Partnership) => p.coordinator_name || p.coordinator_email || 'Unknown Coordinator')
                .filter((name: string) => name && name !== 'Unknown Coordinator');
              coordinatorIds = company.partnerships
                .map((p: Partnership) => p.coordinator_user_id?.toString())
                .filter((id: string | undefined) => id);
              console.log(`✅ Found ${coordinatorNames.length} coordinator(s) from partnerships for company:`, company.name);
            } else if (company.moa_uploaded_by) {
              // Fallback: Fetch coordinator information from old method
              try {
                console.log('📞 Fetching coordinator info for user_id:', company.moa_uploaded_by);
                const coordinatorResponse = await apiService.getCoordinatorProfileByUserId(company.moa_uploaded_by.toString());
                
                console.log('📥 Coordinator API response:', coordinatorResponse);
                
                if (coordinatorResponse.success) {
                  const coord = coordinatorResponse.user || coordinatorResponse.coordinator || coordinatorResponse.data;
                  
                  if (coord) {
                    const firstName = coord.first_name || coord.firstName || '';
                    const lastName = coord.last_name || coord.lastName || '';
                    const email = coord.email || '';
                    
                    const coordinatorName = `${firstName} ${lastName}`.trim() || email || 'Unknown Coordinator';
                    coordinatorNames = [coordinatorName];
                    coordinatorIds = [company.moa_uploaded_by.toString()];
                    
                    console.log('✅ Found coordinator:', coordinatorName, 'for company:', company.name);
                  }
                }
              } catch (error) {
                console.error('❌ Error fetching coordinator info for:', company.moa_uploaded_by, error);
              }
            }
            
            // Format coordinator names for display (comma-separated)
            const coordinatorNameDisplay = coordinatorNames.length > 0 
            ...
            transformedCompanies.push({
              ...
              coordinatorName: coordinatorNameDisplay, // Display all coordinators
              coordinatorId: coordinatorIds.join(', '), // All coordinator IDs
              partnerships: company.partnerships || [], // Store partnerships array
            });
          }
        }
        
        console.log(`✅ Filtered to ${transformedCompanies.length} companies with partnerships`);
        setAllCompanies(transformedCompanies);
      } else {
  ...
  const downloadAllCompaniesExcel = async () => {
    try {
      // Show loading alert
      Alert.alert('Processing', 'Preparing Excel file...');
      
      // Prepare data for Excel from allCompanies
      const excelData = allCompanies.map((company, index) => {
        // Determine partnership status text
        const coordinatorApprovedValue = company.coordinatorApproved;
        const coordinatorApproved = coordinatorApprovedValue === true || 
                                    (typeof coordinatorApprovedValue === 'number' && coordinatorApprovedValue === 1) ||
        ...
        return {
          ...
          'Partnership Date': formatDate(company.partnershipDate),
          'School Year': company.schoolYear || selectedSchoolYear,
          'Coordinator': company.coordinatorName || 'N/A',
          'Website': company.website,
        };
      });
      ...
      if (Platform.OS === 'web') {
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `All_Companies_List_${new Date().toISOString().split('T')[0]}.xlsx`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        window.URL.revokeObjectURL(url);
  ...
  return (
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      <Animated.View style={[styles.searchSection, { transform: [{ translateY: slideAnim }] }]}>
        ...
        <View style={styles.filterContainer}>
          ...
          <View style={styles.actionButtonsRow}>
            <TouchableOpacity 
              style={styles.allCompaniesButton}
              onPress={() => {
            ...
            >
              ...
              <Text style={styles.allCompaniesButtonText}>All Companies</Text>
            </TouchableOpacity>
      ...
      {/* All Companies Table Modal */}
      <Modal
        visible={showAllCompaniesModal}
        animationType="slide"
        transparent={true}
        onRequestClose={() => setShowAllCompaniesModal(false)}
      >
        <View style={styles.allCompaniesModalOverlay}>
          <View style={styles.allCompaniesModalContent}>
            <View style={styles.allCompaniesModalHeader}>
              <View style={styles.allCompaniesHeaderLeft}>
                <MaterialIcons name="business-center" size={24} color="#F56E0F" />
                <Text style={styles.allCompaniesModalTitle}>All Companies</Text>
              </View>
              <View style={styles.allCompaniesHeaderRight}>
                <TouchableOpacity 
                  style={styles.allCompaniesDownloadButton}
                  onPress={downloadAllCompaniesExcel}
                >
                  <MaterialIcons name="download" size={18} color="#fff" />
                  <Text style={styles.allCompaniesDownloadButtonText}>Download Excel</Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  style={styles.allCompaniesCloseButton}
                  onPress={() => setShowAllCompaniesModal(false)}
                >
                  <MaterialIcons name="close" size={24} color="#666" />
                </TouchableOpacity>
              </View>
            ...
            <ScrollView 
              style={styles.allCompaniesTableContainer}
              horizontal
              showsHorizontalScrollIndicator={true}
            >
              <View style={styles.allCompaniesTable}>
                {/* Table Header */}
                <View style={styles.allCompaniesTableHeader}>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 50 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>No.</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 200 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Company Name</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 150 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Industry</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 150 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Location</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 150 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Contact Person</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 200 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Contact Email</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 120 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Contact Phone</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 120 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Available Slots</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 120 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>MOA Status</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 150 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Partnership Status</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 120 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>School Year</Text>
                  </View>
                  <View style={[styles.allCompaniesTableHeaderCell, { width: 180 }]}>
                    <Text style={styles.allCompaniesTableHeaderText}>Coordinator</Text>
                  </View>
                </View>

                {/* Table Body */}
                {loadingAllCompanies ? (
                  <View style={styles.allCompaniesTableLoading}>
                    <ActivityIndicator size="large" color="#F56E0F" />
                    <Text style={styles.allCompaniesTableLoadingText}>Loading companies...</Text>
                  </View>
                ) : allCompanies.length === 0 ? (
                  <View style={styles.allCompaniesTableEmpty}>
                    <MaterialIcons name="business-center" size={48} color="#878787" />
                    <Text style={styles.allCompaniesTableEmptyText}>No companies found</Text>
                  </View>
                ) : (
                  allCompanies.map((company, index) => {
                    ...
                    let partnershipStatus = 'Pending';
                    let partnershipStatusColor = '#878787';
                    if (coordinatorApproved && companyApproved) {
                      partnershipStatus = 'Partner';
                      partnershipStatusColor = '#34a853';
                    } else if (!coordinatorApproved && companyApproved) {
                      partnershipStatus = 'Waiting You';
                      partnershipStatusColor = '#fbbc04';
                    } else if (coordinatorApproved && !companyApproved) {
                      partnershipStatus = 'Waiting Company';
                      partnershipStatusColor = '#fbbc04';
                    }
                    ...
                    return (
                      <View 
                        key={company.id} 
                        style={[
                          styles.allCompaniesTableRow,
                          index % 2 === 0 && styles.allCompaniesTableRowEven
                        ]}
                      >
                        <View style={[styles.allCompaniesTableCell, { width: 50 }]}>
                          <Text style={styles.allCompaniesTableCellText}>{index + 1}</Text>
                        </View>
                        <View style={[styles.allCompaniesTableCell, { width: 200 }]}>
                          <Text style={styles.allCompaniesTableCellText} numberOfLines={1}>
                            {company.name}
                          </Text>
                        </View>
                        <View style={[styles.allCompaniesTableCell, { width: 150 }]}>
                          <Text style={styles.allCompaniesTableCellText} numberOfLines={1}>
                            {company.industry}
                          </Text>
                        </View>
                        ...
                        <View style={[styles.allCompaniesTableCell, { width: 120 }]}>
                          <Text style={styles.allCompaniesTableCellText}>
                            {company.contactPhone}
                          </Text>
                        </View>
                        <View style={[styles.allCompaniesTableCell, { width: 120 }]}>
                          <Text style={styles.allCompaniesTableCellText}>
                            {company.availableSlots}/{company.totalSlots}
                          </Text>
                        </View>
                        <View style={[styles.allCompaniesTableCell, { width: 120 }]}>
                          <View style={[
                            styles.allCompaniesStatusBadge,
                            { backgroundColor: getMOAStatusColor(company.moaStatus) }
                          ]}>
                            <Text style={styles.allCompaniesStatusBadgeText}>
                              {getMOAStatusText(company.moaStatus)}
                            </Text>
                          </View>
                        </View>
                        <View style={[styles.allCompaniesTableCell, { width: 150 }]}>
                          <View style={[
                            styles.allCompaniesStatusBadge,
                            { backgroundColor: partnershipStatusColor }
                          ]}>
                            <Text style={styles.allCompaniesStatusBadgeText}>
                              {partnershipStatus}
                            </Text>
                        ...
                        <View style={[styles.allCompaniesTableCell, { width: 120 }]}>
                          <Text style={styles.allCompaniesTableCellText}>
                            {company.schoolYear || selectedSchoolYear}
                          </Text>
                        </View>
                        <View style={[styles.allCompaniesTableCell, { width: 180 }]}>
                          <Text style={styles.allCompaniesTableCellText} numberOfLines={2}>
                            {company.coordinatorName || 'N/A'}
                          </Text>
                        </View>
                      </View>
                    );
...
const styles = StyleSheet.create({
  ...
  // All Companies Modal Styles
  allCompaniesModalOverlay: {
    flex: 1,
  ...
  allCompaniesModalContent: {
    backgroundColor: '#1B1B1E',
    borderRadius: 16,
    width: '100%',
    maxWidth: width < 768 ? width - 40 : 1400,
    maxHeight: '90%',
    borderWidth: 1,
    borderColor: 'rgba(245, 110, 15, 0.3)',
    overflow: 'hidden',
  },
  allCompaniesModalHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  ...
  allCompaniesHeaderLeft: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
  },
  allCompaniesHeaderRight: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 12,
  },
  allCompaniesModalTitle: {
    fontSize: width < 768 ? 18 : 20,
    fontWeight: 'bold',
    color: '#FBFBFB',
  },
  allCompaniesDownloadButton: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#34a853',
  ...
  allCompaniesDownloadButtonText: {
    color: '#fff',
    fontSize: 14,
    fontWeight: '600',
  },
  allCompaniesCloseButton: {
    padding: 4,
  },
  allCompaniesTableContainer: {
    flex: 1,
    backgroundColor: '#1B1B1E',
  },
  allCompaniesTable: {
  ...
  allCompaniesTableHeaderCell: {
    paddingHorizontal: 8,
    justifyContent: 'center',
  },
  allCompaniesTableHeaderText: {
    fontSize: 13,
    fontWeight: 'bold',
    color: '#FBFBFB',
    textTransform: 'uppercase',
  },
  allCompaniesTableRow: {
    flexDirection: 'row',
    borderBottomWidth: 1,
  ...
  allCompaniesTableRowEven: {
    backgroundColor: 'rgba(255, 255, 255, 0.02)',
  },
  allCompaniesTableCell: {
    paddingHorizontal: 8,
    justifyContent: 'center',
  },
  allCompaniesTableCellText: {
    fontSize: 13,
    color: '#FBFBFB',
    opacity: 0.9,
  },
  allCompaniesStatusBadge: {
    paddingHorizontal: 8,
    paddingVertical: 4,
    borderRadius: 12,
    alignSelf: 'flex-start',
  },
  allCompaniesStatusBadgeText: {
    fontSize: 11,
    fontWeight: 'bold',
    color: '#fff',
  },
  allCompaniesTableLoading: {
    padding: 40,
    alignItems: 'center',
    justifyContent: 'center',
  },
  allCompaniesTableLoadingText: {
    marginTop: 12,
    fontSize: 14,
    color: '#FBFBFB',
    opacity: 0.7,
  },
  allCompaniesTableEmpty: {
    padding: 60,
    alignItems: 'center',
    justifyContent: 'center',
  },
  allCompaniesTableEmptyText: {
    marginTop: 16,
    fontSize: 16,
    color: '#878787',
    fontWeight: '500',
