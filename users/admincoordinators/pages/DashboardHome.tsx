  ...
  export default function DashboardHome({ currentUser }: DashboardHomeProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch data in parallel: companies, interns, partner companies, and all enrolled students
        const [companiesResponse, internsResponse, partnerCompaniesResponse, allEnrolledStudentsResponse] = await Promise.all([
          ...
          apiService.getAllEnrolledStudents() // Get all enrolled students across all classes
        ]);
        ...
        if (companiesResponse.success && companiesResponse.companies && Array.isArray(companiesResponse.companies)) {
          ...
          // Get list of company IDs that are already assigned to the current coordinator
          ...
          if (partnerCompaniesResponse.success && partnerCompaniesResponse.companies && Array.isArray(partnerCompaniesResponse.companies)) {
            ...
            console.log('📋 Companies already assigned to current coordinator:', assignedCompanyIds.size);
          }
          
          // Filter to show companies that are NOT already assigned to the current coordinator
          // NEW LOGIC: Show companies that are NOT partnered with the current coordinator
          // (regardless of their status with other coordinators)
          const companiesWithoutMOA = companiesResponse.companies.filter((company, index) => {
          ...
          // Get total enrolled interns (all students enrolled in classes)
          let totalEnrolledInterns = 0;
          if (allEnrolledStudentsResponse.success && (allEnrolledStudentsResponse as any).count !== undefined) {
            totalEnrolledInterns = (allEnrolledStudentsResponse as any).count;
            console.log('✅ Total enrolled interns count:', totalEnrolledInterns);
          } else {
            console.log('⚠️ Failed to fetch all enrolled students or no data found');
          }
          
          // Calculate stats
          const newStats: InternStats = {
            numberOfInterns,
            totalEnrolledInterns,
      ...
      const animateStep = () => {
        if (currentStep <= steps) {
          ...
          setAnimatedStats({
            ...
            totalEnrolledInterns: Math.floor(targetStats.totalEnrolledInterns * progress),
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsSection}>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statNumber}>{animatedStats.totalEnrolledInterns}</Text>
            <Text style={styles.statLabel}>Total Number of Interns</Text>
            <Text style={styles.statSubLabel}>All Enrolled Students</Text>
          </View>
          <View style={styles.statCard}>
