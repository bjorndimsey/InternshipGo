...
import AllInternsPage from './pages/AllInternsPage';
...
export default function AdminCoordinatorsDashboard({ onLogout, currentUser }: AdminCoordinatorsDashboardProps) {
  ...
  const [showAllInternsPage, setShowAllInternsPage] = useState(false);
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        if (profileResponse.success && profileResponse.user) {
          console.log('🔍 Admin coordinator dashboard profile data:', profileResponse.user);
        ...
        // Fetch notifications
        const adminCoordinatorResponse = await apiService.getAdminCoordinatorProfile(currentUser.id);
        if (adminCoordinatorResponse.success && adminCoordinatorResponse.user) {
          const adminCoordinatorId = adminCoordinatorResponse.user.id;
          const notificationsResponse = await apiService.getCoordinatorNotifications(adminCoordinatorId, currentUser.id) as any;
  ...
  const refreshUserProfile = async () => {
    ...
    try {
      const profileResponse = await apiService.getProfile(currentUser.id);
      if (profileResponse.success && profileResponse.user) {
        console.log('🔄 Refreshing admin coordinator profile:', profileResponse.user);
        setUserProfile({
          first_name: profileResponse.user.first_name || profileResponse.user.firstName || '',
          last_name: profileResponse.user.last_name || profileResponse.user.lastName || '',
          profile_picture: profileResponse.user.profile_picture || profileResponse.user.profilePicture,
  ...
  const renderScreen = () => {
    if (!currentUser) return <DashboardHome currentUser={null} />;
    
    // Show AllInternsPage if flag is set
    if (showAllInternsPage) {
      return (
        <AllInternsPage 
          currentUser={currentUser}
          onBack={() => setShowAllInternsPage(false)}
        />
      );
    }
    ...
    switch (activeScreen) {
      ...
      case 'Interns':
        return (
          <InternsPage 
            currentUser={currentUser}
            onViewAllInterns={() => setShowAllInternsPage(true)}
          />
        );
      ...
      <View style={styles.mainContent}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerContent}>
            ...
            <Text style={styles.headerSubtitle}>Coordinator Admin</Text>
          </View>
            ...
            <View style={styles.userAvatar}>
              {userProfile?.profile_picture ? (
                ...
                <Text style={styles.userAvatarText}>
                  {userProfile?.first_name?.charAt(0) || currentUser?.email?.charAt(0) || 'A'}
