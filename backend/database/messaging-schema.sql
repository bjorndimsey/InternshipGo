-- Messaging system schema for Supabase
-- This extends the existing schema with messaging functionality
...
DROP FUNCTION IF EXISTS create_direct_conversation(BIGINT, BIGINT);
DROP FUNCTION IF EXISTS create_group_conversation(BIGINT, VARCHAR(255), BIGINT[]);
DROP FUNCTION IF EXISTS mark_messages_as_read(BIGINT, BIGINT);
...
DROP FUNCTION IF EXISTS get_user_conversations(BIGINT);
...
-- The messaging system will work with real users from your database
